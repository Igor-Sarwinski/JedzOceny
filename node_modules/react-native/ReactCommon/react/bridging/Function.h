...
class SyncCallback<R(Args...)> {
 ...
 private:
  ...
  template <size_t... Home>
  R apply(std::tuple<Args...> &&args, std::index_sequence<Home...>) const {
    return call(std::move(std::get<Home>(args))...);
  }
...
struct Bridging<butter::function<R(Args...)>> {
 ...
 private:
  ...
  static R callFromJs(
      ...
      const jsi::Value *args,
      const std::shared_ptr<CallInvoker> &jsInvoker,
      std::index_sequence<Home...>) {
    return fn(bridging::fromJs<Args>(rt, args[Home], jsInvoker)...);
  }
};
