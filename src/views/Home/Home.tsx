    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                console.log('Fetched User:', fetchedUser); // Dodaj tę linię
                setUser(fetchedUser);
            } catch (error) {
                console.error('Error fetching user:', error); // Dodaj tę linię
                // Handle error
            }
        };
    ...
    return (
        <View flex={1} marginVertical={15}>
            <View marginBottom={15} backgroundColor={colors.background} height={170} borderRadius={40} flexWrap={'wrap'}
                ...
                <Text style={textStyles}>Liczba twoich opini to: {user?.reviews}</Text>
